---
title: 文件
top_graphic: 1
menu: "main"
menu:
  main:
    weight: 10
lastmod: 2019-05-15
---

# 總覽

* [入門]({{< ref "/getting-started.md" >}})
* [Let's Encrypt 的工作原理]({{< ref "/how-it-works.md" >}})
* [常問問題 (FAQ)]({{< ref "/docs/faq.md" >}})
* [術語表]({{< ref "/docs/glossary.md" >}})

# 使用者資訊

* [ACME 客戶端]({{< ref "/docs/client-options.md" >}})
* [速率限制]({{< ref "/docs/rate-limits.md" >}})
* [憑證過期提醒信件]({{< ref "/docs/expiration-emails.md" >}})

# 使用者進階資訊

* [測試環境]({{< ref "/docs/staging-environment.md" >}})
* [憑證相容性]({{< ref "/docs/cert-compat.md" >}})
* [憑證信任鏈 (根憑證與中間憑證)]({{< ref "/certificates.md" >}})
* [即將推出的功能]({{< ref "/upcoming-features.md" >}})
* [註銷憑證]({{< ref "/docs/revoking.md" >}})
* [憑證頒發機構授權]({{< ref "/docs/caa.md" >}})
* [localhost 憑證]({{< ref "/docs/certificates-for-localhost.md" >}})
* [最佳實踐 – 保持通訊埠 80 為開啟狀態 ]({{< ref "/docs/allow-port-80.md" >}})
* [ACME 驗證方式]({{< ref "/docs/challenge-types.md" >}})
* [憑證透明度紀錄]({{< ref "/docs/ct-logs.html" >}})

# 客戶端開發資訊

* [服務業者的 ACME 客戶端整合指南]({{< ref "/docs/integration-guide.md" >}})
* [ACME 協定更新]({{< ref "/docs/acme-protocol-updates.md" >}})
* [目前 ACME 草案的差異](https://github.com/letsencrypt/boulder/blob/master/docs/acme-divergences.md)
* [尋找帳號 ID]({{< ref "/docs/account-id.md" >}})

