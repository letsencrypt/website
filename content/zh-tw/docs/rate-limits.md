...
我們還有<a name="failed-validations"></a>**網域驗證失敗**限制，對於每個帳號每域名每小時 5 次的限制。在{{<link "測試環境" "/docs/staging-environment" >}}中並沒有那麼嚴格的限制，因此你可以使用測試環境來針對連線問題除錯。
...
<a name="accounts-per-ip-address"></a>**每個 IP 位置**每 3 個小時最多能建立 10 個帳號。**每個 IPv6 /48** 每 3 個小時最多能建立 500 個帳號。要達到這種帳號限制是很難的，我們建議服務提供業者{{<link "使用一個帳號為多個客戶提供服務" "/docs/integration-guide" >}}。
