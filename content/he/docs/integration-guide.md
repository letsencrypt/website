...
[ה־CP/CPS (הצהרת מדיניות האישור) והסכם המנוי](/repository) מציינים שהמנוי הוא מחזיק במפתח הפרטי לאישור. לספקיות אירוח, מדובר בספקית, לא בלקוח של הספקית. אם נמצאת אצלך כרגע בפיתוח תכנית שאנשים מטמיעים בעצמם, אז אלו המטמיעים עצמם.

כתוצאה מכך, אם הנך נציגות מטעם ספקית אחסון, אין צורך לבקש מהלקוחות שלך להסכים להסכם המנויים שלנו. באפשרותך פשוט להנפיק אישורים עבור שמות התחום שבשליטתך ולהתחיל להשתמש בהם.
...
עם זאת, עבור רוב ספקיות האחסון הגדולות יותר אנו ממליצים להשתמש בחשבון בודד ולהגן על מפתח החשבון הזה בקפידה. מצב זה מקל על זיהוי אישורים ששייכים לאותה יישות, וקל יותר לספק התאמות מגבלות מיכסה אם יש צורך בכך. לא תהיה לנו אפשרות להתאים מגבלות מיכסה ביעילות אם נעשה שימוש במספר רב של חשבונות.
...
אם מעניין אותך להשתמש באתגר http-01 בין כה וכה, יכול להיות שעדיף לך להשתמש בהפניות HTTP. ניתן להגדיר כך שכל נקודות ההשקה שלך מול המשתמש (frontends) יפנו מ־`‎/.well-known/acme-challenge/XYZ` אל `validation-server.example.com/XYZ` לכל `XYZ` שהוא. הגדרה שכזאת מעניקה את האחריות להנפקה לשרת תיקוף (`validation-server`) מרכזי, לכן יש להגן על השרת הזה היטב.
...
אנו ממליצים [לבדוק פרטים נוספים בנוגע לפרטי חידוש עם ACME](https://letsencrypt.org/2024/04/25/guide-to-integrating-ari-into-existing-acme-clients/) לכל אחד מהאישורים לפחות פעמיים ביום. נקודת ה־ARI תמליץ מתי לחדש.

כשלב מניעתי לפני ARI, אנו ממליצים לחדש אישורים אוטומטית כשנותר להם עוד שליש מאורך החיים הכולל שלהם. לאישורים עם תקופת תוקף של פחות מ־10 ימים, אנו ממליצים לחדש במחצית אורך החיים הכולל שלהם. עבור האישורים הנוכחיים של Let's Encrypt שתוקפם הוא 90 ימים, משמעות הדבר היא לחדש 30 יום לפני מועד התפוגה.
