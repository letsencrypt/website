...
{{% def id="profile" name="Profile" %}} Un profil est un ensemble de propriétés qui affectent à la fois la validation et le contenu final d'un certificat. Voir la [documentation sur les profils](/docs/profiles) pour une description de chaque profil, de ce qu'il affecte et de la manière de le sélectionner. {{% /def %}}
