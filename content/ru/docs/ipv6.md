...
Например, в описанном выше сценарии повторная попытка не будет выполнена, если на IPv6-адресе есть веб-сервер, но этот веб-сервер не готов ответить на проверочное задание ACME. В этом случае не будет тайм-аута соединения при обращении к IPv6-адресу, и проверочное задание завершится ошибкой без повторной попытки, поскольку был возвращён неверный ответ.
