...
Когда приватный ключ, соответствующий сертификату, становится небезопасным, 
вы должны аннулировать сертификат. Это может произойти по нескольким причинам. 
Например, вы можете случайно выложить приватный ключ на общедоступный сервер, 
хакеры способны получить доступ к вашему приватному ключу на сервере, 
...
сертификат с помощью вашего публичного ключа.
...
не все браузеры проводят эту проверку. Но аннуляция сертификатов, 
соответствующих скомпрометированным приватным ключам, все еще является 
важной практикой и необходимо согласно [Клиентскому соглашению]
...
Нужно подтвердить Let's Encrypt, что вы имеете право на аннуляцию 
...
# Используя приватный ключ сертификата

Если сертификат изначально был выпущен не вами, но у вас есть копия
соответствующего приватного ключа, можно аннулировать сертификат,
используя этот ключ для подписи запроса на аннуляцию. Например,
если вы обнаружили, что приватный ключ случайно попал в общий доступ,
этот метод подойдет для аннуляции соответствующего сертификата,
...
Вам также понадобится копия приватного ключа в PEM-формате. Имея ее, 
...
приватный ключ. Если все же хочется избежать выпуска сертификата, можно 
...
Далее следуйте иснтрукциям. Если вы предпочитаете подтверждать с использованием HTTP, а не DNS, 
...
после чего продолжить процесс аннуляции, как если бы его выпустили вы:
