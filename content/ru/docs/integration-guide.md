...
В нашем [Соглашении](/repository), под термином "Подписчик" мы понимаем любого владельца закрытого ключа для сертификата. Если вы - хостинг-провадер, то подписчиком являетесь вы, а не ваши клиенты. Если вы разрабатываете клиентское ПО, которое пользователь разворачивает и настраивает самостоятельно, то подписчиком будет пользователь, и т.д.

Адрес электронной почты, указываемый при создании аккаунта Let's Encrypt (он же "регистрация") должен принадлежать Подписчику. Мы используем этот адрес для отправки предупреждений об истечении срока действия сертификатов, и уведомлений об изменении нашей [политики конфиденциальности](/privacy).  Если вы - хостинг-провайдер, эти уведомления должны приходить к вам, а не к вашим клиентам. Идеально было бы настроить список рассылки, чтобы несколько сотрудников могли получать от нас уведомления в ваше отстутствие.

Суть в том, что, если вы являетесь хостинг-провайдером, вам не нужно отправлять нам адреса электронной почты ваших клиентов или получать их согласие на наше Соглашение с подписчиком. Вам достаточно выпустить сертификаты для доменов под вашим управлением, и тут же начать их использовать.
...
Наша [политика выпуска сертификатов](/docs/rate-limits) ограничивает число доменных имён для одного сертификата - не более 100. Использовать ли вам отдельный сертификат для каждого доменного имени, или сгруппировать доменные имена для нескольких сертификатов - мы оставляем на ваше усмотрение.
...
Примечание: мы рекомендуем всегда разрешать доступ к вашему веб-серверу по протоколу HTTP с последующим перенаправлением на HTTPS. Для пользователей это будет лучше, чем сброc HTTP-соединения, при том же уровне безопасности.
