...
{{% def id="PKI" name="Public Key Infrastructure" abbr="PKI" %}} A set of roles, policies, and procedures needed to create, manage, distribute, use, store, and revoke digital certificates and manage public-key encryption. The Web PKI is the overall infrastructure that allows public certificate authorities to issue digital certificates that can be accepted automatically by [web clients](#def-web-client). [Wikipedia](https://en.wikipedia.org/wiki/Public_key_infrastructure) {{% /def %}}
