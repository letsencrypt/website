...
{{% def id="OCSP-must-staple" name="OCSP Must-Staple" %}} A [certificate](#def-leaf) property [OID](#def-OID) [1.3.6.1.5.5.7.1.24](https://tools.ietf.org/html/rfc7633#section-6), informing the [browser](#def-web-browser) that the [web server](#def-web-server) with that certificate must use [OCSP stapling](#def-OCSP-staping). It's used to require that an up-to-date [revocation](#def-revocation) status of the [certificate](#def-leaf) is confirmed by the web server on every connection, making revocation more reliable. [Let's Encrypt](#def-LE) can issue certificates with the OCSP Must-Staple property upon request. [Mozilla Security Blog](https://blog.mozilla.org/security/2015/11/23/improving-revocation-ocsp-must-staple-and-short-lived-certificates/) [RFC 7633](https://tools.ietf.org/html/rfc7633) {{% /def %}}
...
{{% def id="web-pki" name="Web PKI" %}} Informally, the collection of software, organizations, and policies that issue and use certificates for HTTPS on the Web. This includes the major browsers, the [root programs](#def-root-program) of the major operating systems, all of the CAs included in those root programs, and the [CA/Browser Forum](#def-CABF). This is distinguished from other uses of certificates, like email encryption and document signing, and from internal-only CAs. {{% /def %}}
