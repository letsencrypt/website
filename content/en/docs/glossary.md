...
{{% def id="AIA" name="Authority Information Access" abbr="AIA" %}} A certificate property used to indicate to [web-clients](#def-web-client) how to obtain information about the issuer of the [certificate](#def-leaf). It typically specifies the [OCSP](#def-OCSP) URI and the [issuer URI](#def-CAI). {{% /def %}}
...
{{% def id="ACME-server" name="ACME Server" %}} An ACME-compatible server that can generate [certificates](#def-leaf). Let's Encrypt's software, [Boulder](boulder), is ACME-compatible, [with some divergences](https://github.com/letsencrypt/boulder/blob/master/docs/acme-divergences.md). {{% /def %}}
...
{{% def id="BRs" name="Baseline Requirements" abbr="BRs" %}} A set of technical and policy requirements for CAs. Since all major [root programs](#def-root-program) incorporate the Baseline Requirements, CAs must abide by these requirements to be trusted by most browsers. {{% /def %}}
...
{{% def id="chain" name="Certificate chain" %}} A list of [intermediate certificates](#def-intermediate) that help a [web client](#def-web-client) determine that it can trust an end-entity or [leaf certificate](#def-leaf), by connecting it to a [root certificate](#def-root) in its [certificate store](#def-store). Note: the path is not always unique, and when a website presents a certificate chain leading to one root, the web client may decide to use another chain, ending in another root, to validate the certificate (This is especially important for Public Key Pinning). [Wikipedia](https://en.wikipedia.org/wiki/Public_key_certificate) {{% /def %}}
...
{{% def id="critical" name="Critical" %}} A certificate may contain fields (also known as [extensions](#def-extension)) marked "critical." This means that software must reject that certificate unless the software understands how to process that extension. This makes it possible to introduce new extensions that are important for security without creating risks for older software. {{% /def %}}

{{% def id="CRL" name="Certificate Revocation List" abbr="CRL" %}} A method to inform [web clients](def-web-client) about the [revocation](#def-revocation) status of a [certificate](#def-leaf). This is a list of the serial numbers of all revoked certificates from a given CA, signed by that CA. [Wikipedia](https://en.wikipedia.org/wiki/Certificate_revocation_list) {{% /def %}}
...
{{% def id="DNSSEC" name="Domain Name System Security Extensions" abbr="DNSSEC" %}} A mechanism to cryptographically authenticate DNS responses. DNSSEC requires deployment by TLDs, domain owners, and recursive resolvers in order to take effect. Adoption is currently somewhat low. [Wikipedia](https://en.wikipedia.org/wiki/Domain_Name_System_Security_Extensions) {{% /def %}}
...
{{% def id="OCSP-must-staple" name="OCSP Must-Staple" %}} A [certificate](#def-leaf) property [OID](#def-OID) [1.3.6.1.5.5.7.1.24](https://tools.ietf.org/html/rfc7633#section-6), informing the [web client](#def-web-client) that the [web server](#def-web-server) with that certificate must use [OCSP stapling](#def-OCSP-staping). It's used to require that an up-to-date [revocation](#def-revocation) status of the [certificate](#def-leaf) is confirmed by the web server on every connection, making revocation more reliable. [Let's Encrypt](#def-LE) can issue certificates with the OCSP Must-Staple property upon request. [Mozilla Security Blog](https://blog.mozilla.org/security/2015/11/23/improving-revocation-ocsp-must-staple-and-short-lived-certificates/) [RFC 7633](https://tools.ietf.org/html/rfc7633) {{% /def %}}

{{% def id="OCSP-stapling" name="OCSP stapling" %}} A way for a [web server](#def-web-server) to send a [web client](#def-web-client) an [OCSP](#def-OCSP) response signed by the [Certificate Authority](#def-CA), so the [Web client](#def-web-client) itself doesn’t need to make a secondary OCSP request to the CA, improving speed and privacy. Also known as TLS Certificate Status Request extension. [Wikipedia](https://en.wikipedia.org/wiki/OCSP_stapling) [Cloudflare](https://blog.cloudflare.com/high-reliability-ocsp-stapling/) {{% /def %}}
...
{{% def id="root" name="Root certificate" %}} A [self-signed](#def-self-signed) certificate controlled by a [certificate authority](#def-CA), used to sign its [intermediates](#def-intermediate) certificates and included in [certificate stores](#def-store). [Wikipedia](https://en.wikipedia.org/wiki/Root_certificate) {{% /def %}}
...
{{% def id="SAN" name="Subject Alternative Name" abbr="SAN" %}} A field of a [certificate](#def-leaf) that indicates for which domain(s) the certificate is valid. It replaces the usage of the [Common Name](#def-CN), which is now provided for compatibility reasons only. A single certificate may contain many SANs and hence be valid for many different domain names. [OID](#def-OID) [2.5.29.17 (RFC 5280)](https://tools.ietf.org/html/rfc5280.html#section-4.2.1.6) - [Wikipedia](https://en.wikipedia.org/wiki/Subject_Alternative_Name) https://letsencrypt.org/docs/rate-limits/#names-per-certificate {{% /def %}}
