...
We allow up to 100 names per certificate depending on the [certificate profile](/docs/profiles/) selected. Whether you use a separate certificate for every hostname, or group together many hostnames on a small number of certificates, is up to you.

Using separate certificates per hostname means fewer moving parts are required to logically add and remove domains as they are provisioned and retired. Separate certificates also minimize certificate size, which can speed up HTTPS handshakes on low-bandwidth networks. Check out our [rate limits](/docs/rate-limits) to make sure you can get as many certificates as you need.
