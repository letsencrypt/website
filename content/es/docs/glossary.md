...
{{% def id="DV" name="Domain-validated certificate" %}} Un [certificado](# def-leaf) donde el solicitante solo ha demostrado su control sobre el nombre de dominio (y no la identidad de la organización solicitante). [Let's Encrypt](# def-LE) ofrece solo certificados DV (no [OV](# def-OV) o [EV](# def-EV)): {{<link "FAQ" "/docs/faq.md" >}} - [Wikipedia](https://en.wikipedia.org/wiki/Domain-validated_certificate) {{% /def %}}
