...
A Let's Encrypt usa o protocolo ACME para verificar que você controla um dado
nome de domínio e para lhe emitir um certificado. Para obter um certificado da
Let's Encrypt, você precisará escolher um cliente ACME para usar.

Os clientes ACME abaixo são oferecidos por terceiros. A Let's Encrypt não controla nem
revisa clientes de terceiros e não pode fazer nenhuma garantia a respeito de segurança 
ou confiabilidade destes clientes.

Alguns clientes ACME utilizáveis diretamente via navegador estão disponíveis, mas nós não os listamos aqui porque
eles encorajam um fluxo de trabalho com renovação manual que resulta em uma experiência de uso ruim
e aumentam o risco do usuário esquecer de renovar seus certificados.

# Recomendado: Certbot

Recomendamos que usuários comecem pelo cliente [Certbot](https://certbot.eff.org/). Ele pode desde simplesmente obter um certificado para você até ajudá-lo a instalar, dependendo do que você quiser fazer. É fácil de usar, funciona em muito sistemas operacionais e tem uma ótima documentação. 

Se o certbot não supre suas necessidades ou você gostaria de tentar algo diferente, existem muitos outros clientes para se escolher na lista abaixo, agrupados por linguagem ou ambiente em que eles são executados.

## Clientes compatíveis com ACME v2

Estes clientes são compatíveis com nosso [ambiente de testes do protocolo ACME v2](https://community.letsencrypt.org/t/staging-endpoint-for-acme-v2/49605) (Inglês).
...
- [Certify The Web (Windows)](https://certifytheweb.com) (v4 e superior)
- [publishlab/node-acme-client](https://github.com/publishlab/node-acme-client)
- [Posh-ACME](https://github.com/rmbolger/Posh-ACME)
- [acme-dns-tiny](https://acme-dns-tiny.adorsaz.ch) (v2.0 e superior)
...
- [GetSSL](https://github.com/srvrco/getssl) (bash, também automatiza certificação em servidores remotos via SSH)
- [acme.sh](https://github.com/Neilpang/acme.sh) (Compatível com bash, dash e sh)
- [dehydrated](https://github.com/lukas2511/dehydrated) (Compatível com bash e zsh)
...
- [Lets-proxy](https://github.com/rekby/lets-proxy) (Proxy reverso com suporte a https/tls)
...
- [GetSSL - Azure Automation](https://www.powershellgallery.com/packages/GetSSL-LetsEncrypt/) (Compatível com qualquer App Service)
...
- [acme_proxy.php](https://github.com/jpawlowski/acme_proxy.php) (Proxy reverso para atender múltiplos servidores internos baseado em um DNS interno)
...
- O módulo Python [acme](https://github.com/certbot/certbot/tree/master/acme)
  é parte das dependências do Certbot, mas também pode ser usado por vários outros
  clientes e está disponível como um pacote separado via [PyPI](https://pypi.python.org/pypi/acme), [Debian](https://packages.debian.org/search?keywords=python-acme),
  [Ubuntu](https://launchpad.net/ubuntu/+source/python-acme), [Fedora](https://bodhi.fedoraproject.org/updates/?packages=python-acme) e outras distribuições.
- [mail-in-a-box/free_tls_certificates](https://github.com/mail-in-a-box/free_tls_certificates) (Python 3)
- [txacme](https://github.com/mithrandi/txacme) (cliente Twisted para Python 2 / 3)
...
# Projetos que usam a Let's Encrypt
...
Se você conhece um cliente ACME ou um projeto que usa a Let's Encrypt que não está presente na lista acima, por favor envie um pull request no nosso [repositório do website](https://github.com/letsencrypt/website/) no Github, atualizando a página `content/pt-br/docs/client-options.md`.

Antes de enviar um pull request certifique-se que:

1. Seu cliente respeita a [política de marca registrada da Let's Encrypt](https://letsencrypt.org/trademarks/) (Inglês).
1. Seu cliente não é feito para ser usado via navegador e que ele suporta renovações automáticas.
1. Seu commit adiciona seu cliente ao **fim** das seções relevantes (não esqueça da seção "Clientes compatíveis com ACME v2" se aplicável!).
1. Seu commit atualiza a data no campo `lastmod` no topo do arquivo `client-options.md`.
