...
要开始使用 Let's Encrypt，请访问我们的[快速入门]({{< ref "/getting-started.md" >}})页面。

## 使用 Let's Encrypt 的费用是多少？真的免费吗？

我们不会对我们的证书收取费用。Let's Encrypt 是一个非营利性组织，我们的任务是通过促进 HTTPS 的广泛采用来创建一个更加安全和尊重隐私的 Web。我们的服务免费且易于使用，因此每个网站都可以使用它来部署 HTTPS。
...
您有大量的 [ACME 客户端]({{< ref "/docs/client-options.md" >}})可以选择。很有可能其中的某些客户端能在新的系统上正常工作。我们建议您在入门时使用 [Certbot](https://certbot.eff.org/)。
