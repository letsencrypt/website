...
每个账户下每个标识符最多可连续验证失败 1152 次。 验证额度的恢复速率为每天可再验证一次，验证成功后限制随即解除。 超过限制后，该账户就无法再为此标识符申请证书。 这种情况下，用户可以通过接口报错信息中的链接前往我们的自助服务页面手动恢复验证，并且可以额外为账户关联的最多 49999 个标识符同时解除限制。
