...
我们曾经使用较长的 URL，例如 `http://ocsp.int-x3.letsensencrypt.org/`。然而，当我们在发布 [新的根证书和中间证书][1]时，我们想要使它们尽可能小一些。 Web 上的每个 HTTPS 连接（每天数十亿）必须发送证书副本，因此每个字节都很重要。我们选择 `lencr.org` 因为它与我们的名称相似：**L**et's **ENCR**ypt。它的发音很像 Terry Pratchett 的 _Discworld_ 小说中名为 [Lancle][] 的虚构区域。
