...
我们每份证书最多支持 100 个域名，具体取决于您所选的[证书配置](/docs/profiles/)。 您可以为每个域名都申请一份证书，也可以只用几份证书覆盖大量域名。

每个域名使用单独的证书可以简化域名增删的流程， 同时减小证书体积，从而在带宽较低的网络上缩短建立 HTTPS 连接所需的时间。 您可以查看[速率限制](/docs/rate-limits)了解可申请的证书数量。

另一方面，一份证书也可以包含大量域名，从而减少需要管理的证书数量。 如果您需要兼容 Windows XP 等不支持 TLS [服务器名称指示](https://zh.wikipedia.org/wiki/%E6%9C%8D%E5%8A%A1%E5%99%A8%E5%90%8D%E7%A7%B0%E6%8C%87%E7%A4%BA) (SNI) 的早期客户端，则每份证书都必须由不同的 IP 地址提供服务，因此每份证书包含的域名越多，您所需要的 IP 地址就越少。

绝大多数情况下，这两种证书管理方式都是安全的。
